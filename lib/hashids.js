...
export default class Hashids {
	...
	constructor(salt = '', minLength = 0, alphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890') {
		...
		this.bignum = (v) => (/^(\-|\+)?([0-9]+|Infinity)$/.test(v)) ? v !== Infinity ? bignum(v) : NaN : NaN;
	...
	decodeHex(id) {
		...
		for (let i = 0; i !== numbers.length; i++) {
			...
			const hexNum = numbers[i].toString(16);
			let sub = 2;
	...
	_encode(numbers) {
		...
		for (let i = 0; i !== numbers.length; i++) {
			...
			if (i + 1 < numbers.length) {
				...
				number = number.mod(last.charCodeAt(0) + i);
				// const sepsIndex = number % this.seps.length;
				const sepsIndex = number.mod(this.seps.length);
		...
		if (ret.length < this.minLength) {
			...
			let guardIndex = numbersIdInt.add(ret[0].charCodeAt(0)).mod(this.guards.length);
	...
	_fromAlphabet(input, alphabet) {
		...
		return input.split("").map(
			...
			(carry, item) => carry.mul(alphabet.length).add(item),
			// 0
			bignum(0)
		);
